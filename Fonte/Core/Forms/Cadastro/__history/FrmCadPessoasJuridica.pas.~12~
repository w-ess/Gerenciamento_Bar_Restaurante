...
begin
  ...
  FDQryFilho.First;
  while not FDQryFilho.Eof do
    begin
      if FDQryFilho.FieldByName('DESCRICAO').AsString = ComboBox1.Text then
        begin
          Atencao('A Classificação já existe para essa pessoa!');
          ComboBox1.SetFocus;
          Abort;
        end;
      FDQryFilho.Next;
...
procedure TFrmCadPessoaJuridica.TabQueryShow(Sender: TObject);
begin
  inherited;
